...
function downloadPDF(frm){
  frappe.call({
            method: "global_vertical.py.quality_check.download_and_attach_task_pdf",
            args: {
                task_name: frm.doc.name
            },
            freeze: true,
            freeze_message: __("Please Wait While We Downloading PDF..."),
            callback: function(r) {
                if (r.message) {
                    const base64Data = r.message;
                    const filename = `${frm.doc.name}.pdf`;

                    // 1. Create a direct download in the browser
                    ...
                    // 2. Show confirmation and reload to show attachment
                    frappe.show_alert({
                        message: "PDF downloaded and attached successfully.",
                        indicator: "green"
                    }, 3);
            ...
            error: function(err) {
                ...
                frappe.show_alert({
                    message: "Failed to download Task PDF.",
                    indicator: "red"
                }, 3);
            }
        });
